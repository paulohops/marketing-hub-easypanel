...
As migrations `0070_notifications_stock_categories.sql` e `0071_notifications_stock_categories_data.sql` separam a criação dos valores enum da criação das tabelas, colunas e índices. Essa divisão evita o uso de valores enum novos antes do commit da transação de migration. O deploy deve aplicá-las pelo fluxo normal, depois da `0069_event_rich_detail.sql`; nenhuma delas foi executada diretamente contra produção.
