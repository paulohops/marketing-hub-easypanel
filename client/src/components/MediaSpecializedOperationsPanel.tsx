...
import { CheckCircle2, Music2, Plus, Radio, Upload, UsersRound } from "lucide-react";
...
export default function MediaSpecializedOperationsPanel({ category, canWrite }: { category: Category; canWrite: boolean }) {
  ...
  if (category === "sound_car") {
    return (
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <PanelHeader icon={Radio} title="Rodagem e spots" description="Registre os dias, horários, rotas e comprovações da operação de carro de som." />
        <div className="grid gap-5 p-5 lg:grid-cols-2">
          <div className="space-y-3">
            {data.data?.spots.slice(0, 4).map(item => <article key={item.id} className="rounded-xl border border-border p-3"><p className="text-sm font-semibold text-foreground">{item.name}</p><p className="mt-1 text-xs text-muted-foreground">{item.active ? "Spot ativo" : "Spot inativo"} · {item.notes || "Sem observações"}</p></article>)}
          </div>
          {canWrite && <form onSubmit={submitRun} className="grid gap-3 rounded-xl bg-secondary p-4 sm:grid-cols-2">
            ...
            <label className="space-y-1"><span className="text-xs font-medium text-foreground">Data</span><Input required type="date" value={run.drivenOn} onChange={event => setRun({ ...run, drivenOn: event.target.value })} /></label>
            ...
            <Textarea className="sm:col-span-2" value={run.route} onChange={event => setRun({ ...run, route: event.target.value })} placeholder="Rota e cidades atendidas" />
            <Textarea className="sm:col-span-2" value={run.notes} onChange={event => setRun({ ...run, notes: event.target.value })} placeholder="Observações e comprovações" />
            ...
            <Button className="sm:col-span-2 bg-primary hover:bg-primary/90">Registrar rodagem</Button>
          </form>}
        </div>
      </section>
    );
