...
export default function ImageViewer({ src, alt, title = "Visualização da imagem", className = "", emptyLabel = "Sem foto", children }: ImageViewerProps) {
  ...
  return <Dialog>
    <DialogTrigger asChild>
      <button type="button" className={`group relative block aspect-square w-20 shrink-0 overflow-hidden rounded-xl border border-border bg-secondary shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${className}`} aria-label={`Ampliar ${alt}`}>
