...
export default function EvidenceUpload({ entityType, entityId, regionalId, canWrite = false }: { entityType: EntityType; entityId: number; regionalId?: number | null; canWrite?: boolean }) {
  ...
  return <div className="mt-3 rounded-xl border border-[#e0e8e2] bg-[#fbfdfb] px-3 py-2.5"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-[11px] font-semibold text-[#60766d]">Evidências e comprovantes</p>{canWrite && <><input ref={inputRef} type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={handleFile} className="hidden" /><Button type="button" variant="outline" size="sm" onClick={() => inputRef.current?.click()} disabled={upload.isPending} className="h-7 rounded-md border-[#d4e4dd] px-2 text-[10px] text-[#437368]">{upload.isPending ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <Paperclip className="mr-1 h-3 w-3" />} Anexar</Button></>}</div>{documents.data?.length ? <div className="mt-2 flex flex-wrap gap-1.5">{documents.data.map(document => <a key={document.id} href={document.url} target="_blank" rel="noreferrer" className="inline-flex max-w-[220px] items-center gap-1 rounded-md bg-[#edf5f1] px-2 py-1 text-[10px] text-[#3d7165] hover:bg-[#e2f0ea]"><FileText className="h-3 w-3 shrink-0" /><span className="truncate">{document.originalName}</span></a>)}</div> : <p className="mt-1 text-[10px] text-[#85958e]">Nenhum arquivo anexado.</p>}</div>;
}
