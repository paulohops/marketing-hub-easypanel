...
afterEach(() => { cleanup(); window.history.replaceState({}, "", "/cadastros/empresas"); vi.clearAllMocks(); vi.restoreAllMocks(); });

describe("empresas fiscais", () => {
  it("confirma e envia a exclusão de uma empresa com desvinculação segura", () => {
    ...
    expect(setup.mocks.deleteRegistry).toHaveBeenCalledWith({ kind: "provider", id: 1 });
    expect(window.location.pathname).toBe("/cadastros/empresas");
  });

  it("mostra e edita matriz, paleta e documentos na ficha de empresa", () => {
    window.history.replaceState({}, "", "/cadastros/empresas/1");
    render(<RegistryEntityWorkspace />);
    ...
    expect(setup.mocks.updateProvider).toHaveBeenCalledWith(expect.objectContaining({ id: 1, headquartersCityId: 3, brandColors: ["#0e723b", "#f45103"] }));
  });

  it("envia a alteração de status da empresa", () => {
    window.history.replaceState({}, "", "/cadastros/empresas/1");
    render(<RegistryEntityWorkspace />);
    fireEvent.click(screen.getByRole("button", { name: "Inativar" }));
    expect(setup.mocks.setRegistryActive).toHaveBeenCalledWith({ kind: "provider", id: 1, active: false });
  });
});
