...
const coordinatesText = (latitude: string, longitude: string) => latitude && longitude ? `${latitude}, ${longitude}` : latitude || longitude;
const parseCoordinatesText = (value: string) => {
  const numbers = value.match(/[-+]?\d+(?:[.,]\d+)?/g)?.map(item => item.replace(",", ".")) ?? [];
...
export default function ActionPointsWorkspace() {
  ...
  const remove = trpc.settings.deleteActionPoint.useMutation({
    onSuccess: () => {
      toast.success("Ponto de ação excluído.");
