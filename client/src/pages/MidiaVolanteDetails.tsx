...
type TraditionalSchedulePayload = Omit<TraditionalScheduleItem, "weekdays" | "specificDates"> & { weekday: number | null };
...
type CalendarSchedule = { programName?: string | null; weekday?: number | null; specificDate?: string | null; neighborhoodName?: string | null; startsAt?: string | null; endsAt?: string | null; notes?: string | null };
type CalendarCampaign = { id: number; name: string; startsOn: string; endsOn: string; status?: string | null; serviceTypeName?: string | null; campaignDetails?: string | null; schedules?: CalendarSchedule[] | null; cityDistributions?: Array<{ cityName?: string | null }> | null; neighborhoodDistributions?: Array<{ neighborhoodName?: string | null; cityName?: string | null }> | null; spots?: CalendarDocument[] | null; evidences?: CalendarDocument[] | null };
...
function MultiOption({ id, label, options, values, onChange, placeholder, createAction }: { id: string; label: string; options: Array<{ id: number; label: string; description?: string }>; values: number[]; onChange: (values: number[]) => void; placeholder: string; createAction?: ReactNode }) {
  return <SearchableMultiSelect id={id} label={label} options={options} values={values} onChange={onChange} maxSelections={id === "volante-signal-neighborhoods" ? 500 : 1} placeholder={placeholder} emptyMessage="Nenhuma opção disponível" createAction={createAction} />;
}
  ...
  const submit = (event: FormEvent) => {
    ...
    const schedules: TraditionalSchedulePayload[] = form.schedules
      ...
      .flatMap((item): TraditionalSchedulePayload[] => {
        ...
        const base = { programName: item.programName.trim(), neighborhoodId: item.neighborhoodId ?? null, notes: item.notes?.trim() || undefined, startsAt, endsAt };
    ...
    if (!schedules.length) { toast.error("Adicione pelo menos uma rodada com data, bairro e horário."); return; }
    if (form.schedules.some(item => item.programName.trim() && !item.specificDates?.length)) { toast.error("Selecione uma ou mais datas no calendário de cada rodada."); return; }
    if (!form.signalNeighborhoodIds.length) { toast.error("Selecione pelo menos um bairro para a rodada."); return; }
    if (neighborhoodOptions.length && form.schedules.some(item => item.programName.trim() && !item.neighborhoodId)) { toast.error("Informe o bairro em cada linha da programação territorial."); return; }
    const payload: TraditionalPayload = { mediaPointId, serviceTypeId: form.serviceTypeId ? Number(form.serviceTypeId) : null, responsibleUserId: form.responsibleUserId ? Number(form.responsibleUserId) : null, tradeCampaignId: form.tradeCampaignId ? Number(form.tradeCampaignId) : null, name: form.name, startsOn: form.startsOn, endsOn: form.endsOn, partnershipType: form.partnershipType, estimatedCost: 0, airingSchedule: form.airingSchedule || undefined, signalNotes: form.signalNotes || undefined, signalNeighborhoodIds: form.signalNeighborhoodIds, campaignDetails: form.campaignDetails || undefined, notes: form.notes || undefined, allowConcurrent: form.allowConcurrent, schedules, confirmReplaceExisting: false };
    ...
    <header className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Ficha operacional de carro ou moto som com contratação, bairros de circulação, calendário de rodagem, comprovações e histórico.</p>
        </div>
