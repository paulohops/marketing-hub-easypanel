...
export default function DataImportWorkspace() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-2xl border border-border bg-card p-6 shadow-sm"><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,24rem)] lg:items-end"><div><h2 className="font-display text-lg font-semibold text-foreground">Módulo de importação</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Selecione no menu qual cadastro deseja importar. Cada opção usa sua própria planilha-modelo e valida os campos do formulário correspondente.</p></div><label className="grid gap-1.5"><span className="text-xs font-medium text-muted-foreground">Cadastro</span><Select value={selectedModule} onValueChange={value => { setSelectedModule(value as ImportModuleId); setPreview(null); }}><SelectTrigger className="h-10 w-full bg-background"><SelectValue placeholder="Selecione um cadastro" /></SelectTrigger><SelectContent>{IMPORT_MODULES.map(module => <SelectItem key={module.id} value={module.id}>{module.label}</SelectItem>)}</SelectContent></Select></label></div><div className="mt-4 rounded-xl bg-secondary p-4"><p className="text-sm font-semibold text-foreground">{definition.label}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{definition.description}</p><p className="mt-2 text-xs text-muted-foreground">Aba da planilha: <strong className="text-foreground">{definition.sheetName}</strong> · {definition.columns.length} campos no modelo.</p></div></section>
