...
function RelationList({ title, items }: { title: string; items: Array<{ id: number; label: string; href: string }> }) {
  return <section className="rounded-[10px] border border-border bg-card p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{title}</p><span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">{items.length}</span></div>{items.length ? <div className="mt-3 grid gap-2">{items.map(item => <Link key={item.id} href={item.href} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-background px-3 py-2 text-sm font-medium text-foreground transition hover:border-primary/40 hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary/30"><span className="min-w-0 truncate">{item.label}</span><Eye aria-hidden="true" className="h-4 w-4 shrink-0 text-muted-foreground" /></Link>)}</div> : <p className="mt-3 rounded-lg border border-dashed border-border px-3 py-2 text-sm text-muted-foreground">Não informado</p>}</section>;
}

function ProviderEditDialogWithoutColors({ provider, scope, pending, onOpenChange, onSubmit, onChange }: { provider: EditableProvider | null; scope: any; pending: boolean; onOpenChange: (open: boolean) => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void; onChange: (next: React.SetStateAction<EditableProvider | null>) => void }) {
  return <Dialog open={provider !== null} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>Editar empresa</DialogTitle><DialogDescription>Atualize os dados institucionais e a matriz, sem alterar os vínculos territoriais já existentes.</DialogDescription></DialogHeader>{provider ? <form className="grid gap-4" onSubmit={onSubmit}><div className="grid gap-4 sm:grid-cols-2"><Field name="name" label="Nome da empresa" defaultValue={provider.name} required /><Field name="legalName" label="Razão social" defaultValue={provider.legalName ?? ""} /><Field name="billingCnpj" label="CNPJ de faturamento" defaultValue={provider.billingCnpj ?? ""} /><Field name="contactName" label="Pessoa de contato" defaultValue={provider.contactName ?? ""} /><Field name="email" label="E-mail" type="email" defaultValue={provider.email ?? ""} /><Field name="phone" label="Telefone" defaultValue={provider.phone ?? ""} /></div><Field name="address" label="Endereço de faturamento" defaultValue={provider.address ?? ""} /><SearchableMultiSelect id="company-headquarters-city" label="Cidade-matriz" maxSelections={1} options={(scope?.cities ?? []).filter((city: any) => city.active).map((city: any) => ({ id: city.id, label: `${city.name} · ${city.state}` }))} values={provider.headquartersCityId ? [provider.headquartersCityId] : []} onChange={values => onChange(current => current ? { ...current, headquartersCityId: values[0] ? Number(values[0]) : null } : current)} placeholder="Selecionar cidade vinculada" emptyMessage="Nenhuma cidade vinculada à Empresa" /><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={pending}>{pending ? "Salvando..." : "Salvar alterações"}</Button></div></form> : null}</DialogContent></Dialog>;
}
