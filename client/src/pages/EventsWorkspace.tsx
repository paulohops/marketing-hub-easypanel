...
function EventDetail({ row, canWrite, post, onPostChange, pending, onBack, onEdit, onOpenCampaign, onStatusChange, onSave, onDelete, deleting }: { row: EventRow; canWrite: boolean; post: EventPostState; onPostChange: (value: EventPostState) => void; pending: boolean; onBack: () => void; onEdit: () => void; onOpenCampaign: (campaignId: number) => void; onStatusChange: (status: EventStatus) => void; onSave: () => void; onDelete: () => void; deleting: boolean }) {
  const { event } = row;
