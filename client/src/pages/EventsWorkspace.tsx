...
import { ArrowLeft, Handshake, MapPinned, PackageCheck, Plus, RefreshCw, Search, SlidersHorizontal, UsersRound } from "lucide-react";
    ...
    <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
      <DialogContent className="max-h-[88vh] w-[calc(100vw-2rem)] max-w-6xl overflow-y-auto p-6 sm:p-7">
        ...
        <form onSubmit={submit} className="mt-2 grid gap-4 md:grid-cols-4">
          ...
          <div className="md:col-span-2"><SearchableMultiSelect id="event-services" label="Serviços" options={(references.data?.serviceTypes ?? []).map(service => ({ id: service.id, label: service.name }))} values={form.serviceTypeIds} onChange={ids => setForm({ ...form, serviceTypeIds: ids })} createAction={<InlineRegistryCreateDialog kind="service" onCreated={record => setForm(current => ({ ...current, serviceTypeIds: [...current.serviceTypeIds, record.id] }))} />} /></div>
