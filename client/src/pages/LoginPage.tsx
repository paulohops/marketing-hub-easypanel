...
import { ArrowLeft, CheckCircle2, KeyRound, Loader2, LockKeyhole, MailCheck } from "lucide-react";
...
export default function LoginPage() {
  ...
  const login = trpc.auth.local.login.useMutation({ onSuccess: () => setStage("login-code"), onError: error => toast.error(error.message) });
  ...
  const submitCode = (event: React.FormEvent) => { event.preventDefault(); verifyLoginCode.mutate({ email, code }); };
      ...
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1440px] overflow-hidden rounded-[10px] border border-border bg-card shadow-[0_32px_80px_rgba(14,114,59,0.12)] sm:min-h-[calc(100vh-3.5rem)] lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="flex items-center justify-center px-7 py-12 sm:px-14">
          <div className="w-full max-w-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">Acesse com sua conta local. Por segurança, um código de confirmação será enviado ao seu e-mail antes da entrada.</p>
            ...
            {stage === "login-code" && <form className="mt-7 space-y-4" onSubmit={submitCode}><div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground"><MailCheck className="mb-2 h-5 w-5 text-primary" />Enviamos um código de 6 dígitos para <strong className="text-foreground">{email}</strong>.</div><div className="space-y-2"><label className="text-sm font-semibold text-foreground" htmlFor="login-code">Código de acesso</label><Input id="login-code" inputMode="numeric" autoComplete="one-time-code" maxLength={6} required value={code} onChange={event => setCode(event.target.value.replace(/\D/g, ""))} placeholder="000000" /></div><Button type="submit" disabled={verifyLoginCode.isPending} className="h-12 w-full bg-primary text-primary-foreground">{verifyLoginCode.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <MailCheck className="mr-2 h-4 w-4" />}Confirmar acesso</Button><button type="button" className="inline-flex items-center text-sm font-semibold text-primary" onClick={() => setStage("credentials")}><ArrowLeft className="mr-1 h-4 w-4" />Voltar</button></form>}
