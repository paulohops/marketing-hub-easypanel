...
const campaign = {
  ...
  tradeCampaignName: "Campanha Primavera", evidences: [], history: [],
};
...
describe("ficha de veiculação urbana", () => {
  it("apresenta dados operacionais, status, debriefing e evidências", () => {
    ...
    expect(screen.getByText("Anexar evidência")).toBeInTheDocument();
  });

  it("atualiza diretamente o status planejado", () => {
    window.history.replaceState({}, "", "/midias/veiculacao/13");
    campaignDetailsQuery.mockReturnValue({ data: campaign, isLoading: false });
    render(<UrbanVeiculationPage />);
    fireEvent.click(screen.getByRole("button", { name: /Planejada/ }));
    expect(updateStatusMutation).toHaveBeenCalledWith({ campaignId: 13, status: "scheduled" });
  });
});
