...
import { BellRing, Building2, Check, ChevronRight, CircleHelp, Globe2, Loader2, Mail, MapPin, Pencil, Plus, Save, ShieldCheck, Trash2, UserRound, UsersRound } from "lucide-react";
...
type Draft = {
  ...
  description: string;
  entityType: string;
  eventType: "created" | "assigned" | "updated" | "status_changed" | "deleted" | "due" | "expiry" | "*";
...
const eventLabels: Record<Draft["eventType"], string> = { created: "Registro criado", assigned: "Tarefa atribuída", updated: "Registro alterado", status_changed: "Status alterado", deleted: "Registro excluído", due: "Prazo vencendo", expiry: "Vencimento próximo", "*": "Qualquer evento" };
...
export default function NotificationSettingsWorkspace() {
  ...
  const emailStatus = references?.emailStatus;
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    {emailStatus && <section className={`mt-3 rounded-2xl border p-4 ${emailStatus.ready ? "border-emerald-200 bg-emerald-50/70" : "border-amber-200 bg-amber-50/70"}`}><div className="flex items-start gap-3"><Mail className={`mt-0.5 h-5 w-5 shrink-0 ${emailStatus.ready ? "text-emerald-700" : "text-amber-700"}`} /><div className="min-w-0 flex-1"><h2 className="font-display text-sm font-semibold text-foreground">Canal de e-mail</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{emailStatus.ready ? "SMTP global habilitado e configurado. Regras com e-mail enviarão para os destinatários com endereço cadastrado." : "O canal de e-mail ainda não está pronto. Habilite as notificações por e-mail e configure SMTP em Configurações › Sistema."}</p></div><Badge className={emailStatus.ready ? "rounded-full bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : "rounded-full bg-amber-100 text-amber-900 hover:bg-amber-100"}>{emailStatus.ready ? "Pronto" : "Pendente"}</Badge></div></section>}
