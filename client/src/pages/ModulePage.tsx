...
export default function ModulePage({ eyebrow, title, description, icon: Icon, resources, accent = "var(--primary)" }: ModulePageProps) {
  return (
    <WorkspaceShell>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {resources.map((resource, index) => (
            <WorkspaceCard key={resource.title} className="hub-card--padded hub-card--interactive">
              ...
              <h2 className="mt-0 font-display text-lg font-semibold text-foreground">{resource.title}</h2>
              <p className="mt-2 text-xs leading-5 text-muted-foreground">{resource.description}</p>
              <button onClick={() => toast.info("Este painel será conectado aos registros reais da sua operação.")} className="mt-4 flex items-center gap-1 text-xs font-semibold text-primary">
      ...
      <Empty className="hub-empty-state min-h-[20rem] bg-card">
        <EmptyHeader>
        ...
        <Button variant="outline" className="mt-0" onClick={() => toast.info("Configure os cadastros-base antes de incluir registros operacionais.")}>Conhecer o fluxo</Button>
      </Empty>
      <div className="hub-status-note flex items-center gap-2">
