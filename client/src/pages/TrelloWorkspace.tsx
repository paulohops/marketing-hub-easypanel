...
export default function TrelloWorkspace() {
  ...
  const embedUrl = configuration.data?.embedUrl ?? boardUrl;
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    {boardUrl ? <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm"><iframe title="Quadro Trello do Trade HUB" src={embedUrl} className="h-[min(72vh,900px)] min-h-[540px] w-full bg-background" allow="clipboard-write" referrerPolicy="strict-origin-when-cross-origin" /></section> : <section className="grid min-h-[360px] place-items-center rounded-2xl border border-dashed border-border bg-card p-8 text-center"><div className="max-w-md"><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-secondary text-primary"><LayoutDashboard className="h-5 w-5" /></span><h2 className="mt-4 font-display text-xl font-semibold text-foreground">Nenhum quadro conectado</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{canConfigure ? "Conecte a URL do quadro para disponibilizá-lo a toda a equipe." : "Peça a uma pessoa administradora para conectar o quadro Trello deste ambiente."}</p></div></section>}
    <Dialog open={isConfigurationOpen} onOpenChange={setIsConfigurationOpen}><DialogContent className="max-w-xl"><DialogHeader><DialogTitle>Configurar quadro compartilhado</DialogTitle><DialogDescription>Informe uma URL de quadro Trello que permita incorporação. Pessoas com quadro exclusivo atribuído manterão sua configuração individual.</DialogDescription></DialogHeader><form className="grid gap-4 pt-2" onSubmit={event => { event.preventDefault(); updateConfiguration.mutate({ url: draftUrl }, { onSuccess: () => setIsConfigurationOpen(false) }); }}><div><Label htmlFor="trello-url">URL do quadro</Label><Input id="trello-url" className="mt-1.5" value={draftUrl} onChange={event => setDraftUrl(event.target.value)} placeholder="https://trello.com/b/..." /></div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setIsConfigurationOpen(false)}>Cancelar</Button><Button type="submit" disabled={updateConfiguration.isPending}><Save className="mr-2 h-4 w-4" />Salvar</Button></div></form></DialogContent></Dialog>
  </div>;
}
