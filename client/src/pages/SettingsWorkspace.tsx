...
import { BadgeCheck, Landmark, LockKeyhole, Settings2, ShieldCheck, UsersRound } from "lucide-react";
...
const administrativeAreas = [
  ...
  { title: "Segurança de acesso", description: "A autenticação é realizada pelo Manus OAuth, com sessões protegidas e senhas mantidas exclusivamente pelo provedor.", icon: LockKeyhole, action: "Política OAuth ativa" },
...
export default function SettingsWorkspace() {
  return <div className="mx-auto max-w-5xl">
    <div className="flex gap-4 border-b border-border pb-6"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-primary text-white shadow-sm"><Settings2 className="h-5 w-5" /></span><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground">Administração do sistema</p><h1 className="mt-1 font-display text-3xl font-semibold tracking-tight text-foreground">Configurações</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Central de acesso, segurança e governança do Trade HUB — Cluster MG.</p></div></div>
    <section className="mt-6 rounded-2xl border border-primary/15 bg-primary/5 p-5"><div className="flex gap-3"><Landmark className="mt-0.5 h-5 w-5 shrink-0 text-primary" /><div><h2 className="font-display text-lg font-semibold text-foreground">Cadastros operacionais em seus contextos</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Regionais, cidades, fornecedores, parceiros, mídias, serviços e tipos de ação são administrados nos módulos onde serão utilizados. Essa separação reduz erros de cadastro e mantém cada decisão próxima ao fluxo de trabalho correspondente.</p></div></div></section>
