...
const definitions = {
  ...
  configuracoes: { permission: "settings.read", eyebrow: "Administração do sistema", title: "Configurações", description: "Gerencie parâmetros globais, segurança e usuários. Os cadastros operacionais são realizados nos módulos correspondentes.", icon: Settings2, resources: [{ title: "Segurança", description: "Papéis, permissões e rastreabilidade." }, { title: "Usuários", description: "Acessos e administração da equipe." }, { title: "Parâmetros globais", description: "Diretrizes do ambiente protegido." }], accent: "var(--primary)" },
...
export default function ProtectedModule({ module }: { module: keyof typeof definitions }) {
  ...
  if (!hasReadPermission || (module === "usuarios" && user?.role !== "admin")) {
