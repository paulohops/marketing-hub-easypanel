...
A solução deve reutilizar `DashboardLayout`, tokens semânticos, componentes de `client/src/components/ui`, o padrão lista → detalhe → diálogo/formulário e os hooks de permissão e densidade. Para identidade visual, use `useBranding()`, `shared/branding.ts` e `appSettings`; não leia a configuração diretamente no componente nem crie uma segunda paleta. Não crie uma segunda navegação, um segundo sistema de cores, CSS global específico ou uma rota paralela para contornar o fluxo existente.
