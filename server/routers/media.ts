...
export const mediaRouter = router({
  ...
  updateUrbanVeiculation: protectedProcedure.input(z.object({ campaignId: z.number().int().positive(), serviceTypeId: z.number().int().positive(), responsibleUserId: z.number().int().positive().nullable().optional(), tradeCampaignId: z.number().int().positive().nullable().optional(), name: z.string().trim().min(2).max(180), startsOn: z.string().date(), endsOn: z.string().date(), campaignDetails: z.string().trim().min(3).max(4000) })).mutation(async ({ ctx, input }) => {
    await assertPermission(ctx.user, "media.write");
    if (input.endsOn < input.startsOn) throw new TRPCError({ code: "BAD_REQUEST", message: "A data final deve ser posterior à data inicial." });
    const database = await requireDatabase();
