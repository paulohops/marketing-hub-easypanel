...
export const mediaRouter = router({
  ...
  referenceData: protectedProcedure.query(async ({ ctx }) => {
    await assertPermission(ctx.user, "media.read");
    const database = await requireDatabase();
    const [supplierRows, cityRows, regionalRows, mediaTypeRows, serviceRows, relationRows, supplierMediaRows, supplierCityRows, supplierServiceRows, offeringRows, contractRows, userRows, tradeCampaignRows] = await Promise.all([
  ...
  pointDetails: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive() })).query(async ({ ctx, input }) => {
    ...
    const [pointHistory, campaignHistory, evidences, pointDocuments, distributions, spotRows, scheduleRows, registrations] = await Promise.all([
      ...
      campaignIds.length ? database.select().from(mediaCampaignSchedules).where(inArray(mediaCampaignSchedules.mediaCampaignId, campaignIds)).orderBy(asc(mediaCampaignSchedules.startsAt)) : Promise.resolve([]),
  ...
  createTraditionalVeiculation: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive(), serviceTypeId: z.number().int().positive().nullable().optional(), responsibleUserId: z.number().int().positive().nullable().optional(), tradeCampaignId: z.number().int().positive().nullable().optional(), name: z.string().trim().min(2).max(180), startsOn: z.string().date(), endsOn: z.string().date(), partnershipType: z.enum(partnershipKinds).default("paid"), estimatedCost: z.number().min(0).default(0), notes: z.string().trim().max(2000).optional(), campaignDetails: z.string().trim().max(4000).optional(), airingSchedule: z.string().trim().max(2000).optional(), signalNotes: z.string().trim().max(3000).optional(), signalCityIds: z.array(z.number().int().positive()).max(100).default([]), schedules: z.array(traditionalScheduleSchema).max(100).default([]), allowConcurrent: z.boolean().default(false), confirmReplaceExisting: z.boolean().default(false) })).mutation(async ({ ctx, input }) => {
    ...
    const created = await database.transaction(async transaction => {
      ...
      if (input.schedules.length) await transaction.insert(mediaCampaignSchedules).values(input.schedules.map(schedule => ({ mediaCampaignId: campaign.id, programName: schedule.programName, weekday: schedule.weekday ?? null, specificDate: schedule.specificDate ?? null, startsAt: schedule.startsAt, endsAt: schedule.endsAt, notes: schedule.notes || null, createdByUserId: ctx.user.id })));
      return campaign;
    });
    await writeAuditLog({ actorUserId: ctx.user.id, entityType: "media_campaign", entityId: created.id, action: status === "scheduled" ? "schedule" : "create", afterData: { ...created, signalCityIds: input.signalCityIds, replacedCampaignIds: input.confirmReplaceExisting ? blockingCampaignIds : [], schedules: input.schedules } });
    return created;
  }),
