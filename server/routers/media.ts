...
export const mediaRouter = router({
  ...
  renewCampaign: protectedProcedure.input(z.object({ sourceCampaignId: z.number().int().positive(), name: z.string().trim().min(2).max(180), startsOn: z.string().date(), endsOn: z.string().date(), partnershipType: z.enum(partnershipKinds).default("paid"), estimatedCost: z.number().min(0).default(0), notes: z.string().trim().max(2000).optional(), campaignDetails: z.string().trim().max(4000).optional(), rescheduleReason: z.string().trim().min(3).max(2000).optional(), evidenceDocumentIds: z.array(z.number().int().positive()).max(10).default([]) })).mutation(async ({ ctx, input }) => {
    ...
    const result = await database.transaction(async transaction => {
      ...
      if (input.evidenceDocumentIds.length) {
        const evidenceRows = await transaction.select({ id: documents.id }).from(documents).where(and(eq(documents.entityType, "media_campaign"), eq(documents.entityId, source.id), eq(documents.kind, "evidence"), inArray(documents.id, input.evidenceDocumentIds)));
      ...
      const [updated] = await transaction.update(mediaCampaigns).set({ name: input.name, startsOn: input.startsOn, endsOn: input.endsOn, partnershipType: input.partnershipType, estimatedCost: input.estimatedCost.toFixed(2), notes: input.notes || null, campaignDetails: input.campaignDetails || null, rescheduleReason: input.rescheduleReason || "Reagendamento de veiculação", status: campaignStatusFor(input.startsOn), updatedAt: new Date() }).where(eq(mediaCampaigns.id, source.id)).returning();
