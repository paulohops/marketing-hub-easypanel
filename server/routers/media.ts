...
export const mediaRouter = router({
  ...
  referenceData: protectedProcedure.query(async ({ ctx }) => {
    await assertPermission(ctx.user, "media.read");
    const database = await requireDatabase();
    const [supplierRows, cityRows, regionalRows, mediaTypeRows, serviceRows, subserviceRows, relationRows, serviceSubserviceRows, catalogRows, productRows, supplierMediaRows, supplierCityRows, supplierServiceRows, offeringRows, contractRows, userRows, tradeCampaignRows] = await Promise.all([
  ...
  createUrbanVeiculation: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive(), urbanMediaRegistrationId: z.number().int().positive().optional(), serviceTypeId: z.number().int().positive(), subserviceTypeId: z.number().int().positive().nullable().optional(), mediaServiceCatalogId: z.number().int().positive().nullable().optional(), productTypeId: z.number().int().positive().nullable().optional(), responsibleUserId: z.number().int().positive().nullable().optional(), tradeCampaignId: z.number().int().positive().nullable().optional(), name: z.string().trim().min(2).max(180), objective: z.string().trim().max(180).optional(), startsOn: z.string().date(), endsOn: z.string().date(), partnershipType: z.enum(partnershipKinds).default("paid"), estimatedCost: z.number().min(0).default(0), notes: z.string().trim().max(2000).optional(), campaignDetails: z.string().trim().max(4000).optional() })).mutation(async ({ ctx, input }) => {
    await assertPermission(ctx.user, "media.write");
