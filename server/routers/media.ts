...
async function assertPointRelationships(database: Awaited<ReturnType<typeof requireDatabase>>, input: { supplierId: number; cityId: number; mediaTypeId: number; mediaVariationTypeId?: number | null; serviceTypeId: number | null; operationCategory: typeof mediaOperationCategories[number] }) {
...
export const mediaRouter = router({
  ...
  pointDetails: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive() })).query(async ({ ctx, input }) => {
    ...
    const [pointHistory, campaignHistory, evidences, distributions, spotRows, registrations] = await Promise.all([
    ...
    const shapedCampaigns = campaigns.map(campaign => ({ ...campaign, evidences: evidences.filter(document => document.entityId === campaign.id), cityDistributions: distributions.filter(item => item.distribution.mediaCampaignId === campaign.id).map(item => ({ ...item.distribution, cityName: item.cityName, regionalName: item.regionalName })) }));
    return { ...row.point, supplierName: row.supplierName, cityName: row.cityName, regionalId: row.regionalId, regionalName: row.regionalName, mediaTypeName: row.mediaTypeName, serviceTypeName: row.serviceTypeName, spots: spotRows, campaigns: shapedCampaigns, registrations: registrations.map(item => ({ ...item.registration, variationName: item.variationName, contractCode: item.contractCode, contractName: item.contractName, contractExpectedAmount: item.contractExpectedAmount, veiculations: shapedCampaigns.filter(campaign => campaign.urbanMediaRegistrationId === item.registration.id) })), history: [...pointHistory.map(item => ({ ...item, scope: "point" as const })), ...campaignHistory.map(item => ({ ...item, scope: "campaign" as const }))].sort((first, second) => second.occurredAt.getTime() - first.occurredAt.getTime()) };
  }),
  createPoint: protectedProcedure.input(z.object({ supplierId: z.number().int().positive(), cityId: z.number().int().positive(), mediaTypeId: z.number().int().positive(), mediaVariationTypeId: z.number().int().positive().nullable().optional(), serviceTypeId: z.number().int().positive().nullable(), name: z.string().trim().min(2).max(180), operationCategory: z.enum(mediaOperationCategories).default("graphics"), replacementFrequency: z.enum(["weekly", "monthly", "quarterly", "semiannual", "annual", "custom"]).nullable().optional(), address: z.string().trim().max(2000).optional(), latitude: z.number().min(-90).max(90).nullable(), longitude: z.number().min(-180).max(180).nullable() })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "media.write"); const database = await requireDatabase(); await assertPointRelationships(database, input); const [created] = await database.insert(mediaPoints).values({ ...input, channelKind: input.operationCategory === "graphics" || input.operationCategory === "audio_video" || input.operationCategory === "influencers" ? "standard" : "external", mediaVariationTypeId: input.mediaVariationTypeId ?? null, replacementFrequency: input.replacementFrequency ?? null, address: input.address || null, latitude: input.latitude?.toFixed(7) ?? null, longitude: input.longitude?.toFixed(7) ?? null }).returning(); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "media_point", entityId: created.id, action: "create", afterData: created }); return created; }),
  createUrbanRegistration: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive(), mediaVariationTypeId: z.number().int().positive(), supplierContractId: z.number().int().positive().nullable().optional(), replacementFrequency: z.enum(["weekly", "monthly", "quarterly", "semiannual", "annual", "custom"]), contractReference: z.string().trim().max(180).optional(), contractValue: z.number().min(0).optional() })).mutation(async ({ ctx, input }) => {
  ...
  createUrbanVeiculation: protectedProcedure.input(z.object({ urbanMediaRegistrationId: z.number().int().positive(), tradeCampaignId: z.number().int().positive().nullable().optional(), name: z.string().trim().min(2).max(180), objective: z.string().trim().min(2).max(180), startsOn: z.string().date(), endsOn: z.string().date(), partnershipType: z.enum(partnershipKinds).default("paid"), estimatedCost: z.number().min(0).optional(), notes: z.string().trim().max(2000).optional(), campaignDetails: z.string().trim().max(4000).optional() })).mutation(async ({ ctx, input }) => {
    await assertPermission(ctx.user, "media.write");
    ...
    const [created] = await database.insert(mediaCampaigns).values({ mediaPointId: registration.mediaPointId, urbanMediaRegistrationId: registration.id, mediaVariationTypeId: registration.mediaVariationTypeId, tradeCampaignId: input.tradeCampaignId ?? null, name: input.name, objective: input.objective, startsOn: input.startsOn, endsOn: input.endsOn, partnershipType: input.partnershipType, estimatedCost: (input.estimatedCost ?? Number(registration.contractValue)).toFixed(2), notes: input.notes || null, campaignDetails: input.campaignDetails || null, status }).returning();
  ...
  createConfiguredCampaign: protectedProcedure.input(z.object({ mediaPointId: z.number().int().positive(), tradeCampaignId: z.number().int().positive().nullable().optional(), name: z.string().trim().min(2).max(180), startsOn: z.string().date(), endsOn: z.string().date(), partnershipType: z.enum(partnershipKinds).default("paid"), estimatedCost: z.number().min(0).default(0), notes: z.string().trim().max(2000).optional(), campaignDetails: z.string().trim().max(4000).optional(), campaignConfig: z.object({ dailyRate: z.number().min(0).optional(), circulationDays: z.number().int().min(1).max(366).optional(), dailyRoute: z.string().trim().max(2000).optional(), audioBrief: z.string().trim().max(4000).optional(), vehicleOperation: z.string().trim().max(2000).optional(), airingSchedule: z.string().trim().max(4000).optional(), activeSpotId: z.number().int().positive().optional(), materialFormat: z.string().trim().max(160).optional(), materialQuantity: z.number().int().min(1).optional(), deadlineDays: z.number().int().min(0).max(730).optional(), deliveryInstructions: z.string().trim().max(4000).optional() }).default({}), cityDistributions: z.array(z.object({ cityId: z.number().int().positive(), quantity: z.number().int().min(1), notes: z.string().trim().max(1000).optional() })).max(100).default([]) })).mutation(async ({ ctx, input }) => {
    ...
    const created = await database.transaction(async transaction => { const [campaign] = await transaction.insert(mediaCampaigns).values({ mediaPointId: input.mediaPointId, tradeCampaignId: input.tradeCampaignId ?? null, name: input.name, startsOn: input.startsOn, endsOn: input.endsOn, partnershipType: input.partnershipType, estimatedCost: input.estimatedCost.toFixed(2), notes: input.notes || null, campaignDetails: input.campaignDetails || null, campaignConfig: input.campaignConfig, status }).returning(); if (input.cityDistributions.length) await transaction.insert(mediaCampaignCityDistributions).values(input.cityDistributions.map(item => ({ mediaCampaignId: campaign.id, cityId: item.cityId, quantity: item.quantity, notes: item.notes || null }))); return campaign; });
