...
export function canUpdateActionStatus(currentStatus: string, nextStatus: string) {
  return ["planned", "in_progress", "completed", "cancelled"].includes(currentStatus) && ["planned", "in_progress", "completed", "cancelled"].includes(nextStatus);
}
...
export const actionsRouter = router({
  referenceData: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [cityRows, typeRows, supplierRows, serviceRows, supervisorRows, teamRows, stockRows, actionPointRows, supplierCityRows, campaignRows, eventRows, offeringRows] = await Promise.all([
    ...
    return { cities: cityRows, actionTypes: typeRows, suppliers: supplierRows, serviceTypes: serviceRows, supervisors: supervisorRows, teamUsers: teamRows, stockItems: stockRows, actionPoints: actionPointRows, supplierCities: supplierCityRows, campaigns: campaignRows, events: eventRows, supplierOfferings: offeringRows };
  }),
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    return rows.map(row => {
      ...
      const estimatedAmount = linkedServices.reduce((total, service) => total + Number(service.estimatedAmount), 0);
  ...
  create: protectedProcedure.input(z.object({
    ...
    const estimatedCost = serviceAllocations.reduce((total, allocation) => total + allocation.estimatedAmount, 0);
  ...
  updateExecutionStatus: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), status: z.enum(["planned", "in_progress", "completed", "cancelled"]) })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); const database = await requireDatabase(); const [before] = await database.select().from(actions).where(eq(actions.id, input.actionId)).limit(1); if (!before) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); if (!canUpdateActionStatus(before.status, input.status)) throw new TRPCError({ code: "BAD_REQUEST", message: "Status de ação inválido." }); const [updated] = await database.update(actions).set({ status: input.status, updatedAt: new Date() }).where(eq(actions.id, input.actionId)).returning(); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: updated.id, action: "update_execution_status", beforeData: before, afterData: updated }); return updated; }),
