...
const actionListInput = z.object({
...
export type ActionStatus = "planned" | "in_progress" | "paused" | "completed" | "cancelled";

const allowedActionStatusTransitions: Record<ActionStatus, readonly ActionStatus[]> = {
...
export function canUpdateActionStatus(currentStatus: string, nextStatus: string) {
  return (allowedActionStatusTransitions[currentStatus as ActionStatus] ?? []).includes(nextStatus as ActionStatus);
}
...
export const actionsRouter = router({
  ...
  list: protectedProcedure.input(actionListInput).query(async ({ ctx, input }) => {
    await assertPermission(ctx.user, "actions.read");
    const database = await requireDatabase();
    const filters = [];
    if (input?.actionId) filters.push(eq(actions.id, input.actionId));
    if (input?.status) filters.push(eq(actions.status, input.status));
    if (input?.regionalId) filters.push(eq(cities.regionalId, input.regionalId));
    if (input?.cityId) filters.push(eq(actions.cityId, input.cityId));
    if (input?.supervisorId) filters.push(eq(actions.commercialSupervisorId, input.supervisorId));
    if (input?.rating) filters.push(eq(actionDebriefs.rating, input.rating));
    if (input?.search) {
      const pattern = `%${input.search}%`;
      filters.push(or(ilike(actions.name, pattern), ilike(actions.objective, pattern), ilike(cities.name, pattern), ilike(actionTypes.name, pattern))!);
    }
    const where = filters.length ? and(...filters) : undefined;
    const actionQuery = database.select({ action: actions, cityName: cities.name, actionTypeName: actionTypes.name, debrief: actionDebriefs, supervisorName: commercialSupervisors.name, actionPointName: actionPoints.name, campaignName: tradeCampaigns.name, campaignLogoUrl: tradeCampaigns.logoUrl, eventName: events.name }).from(actions).innerJoin(cities, eq(actions.cityId, cities.id)).innerJoin(actionTypes, eq(actions.actionTypeId, actionTypes.id)).leftJoin(actionDebriefs, eq(actionDebriefs.actionId, actions.id)).leftJoin(commercialSupervisors, eq(actions.commercialSupervisorId, commercialSupervisors.id)).leftJoin(actionPoints, eq(actions.actionPointId, actionPoints.id)).leftJoin(tradeCampaigns, eq(actions.tradeCampaignId, tradeCampaigns.id)).leftJoin(events, eq(actions.eventId, events.id)).where(where).orderBy(asc(actions.scheduledFor), asc(actions.id));
    const rows = input ? await actionQuery.limit(input.pageSize).offset((input.page - 1) * input.pageSize) : await actionQuery;
    const actionIds = rows.map(row => row.action.id);
    if (!actionIds.length) return [];
    const [teamRows, stockRows, supplierRows, serviceRows, historyRows, imageRows, invoiceRows] = await Promise.all([
      database.select({ actionId: actionTeamMembers.actionId, userId: users.id, name: users.name, jobTitle: users.jobTitle, avatarUrl: users.avatarUrl }).from(actionTeamMembers).innerJoin(users, eq(actionTeamMembers.userId, users.id)).where(inArray(actionTeamMembers.actionId, actionIds)),
      database.select({ actionId: actionStockItems.actionId, stockItemId: stockItems.id, name: stockItems.name, unit: stockItems.unit, plannedQuantity: actionStockItems.plannedQuantity }).from(actionStockItems).innerJoin(stockItems, eq(actionStockItems.stockItemId, stockItems.id)).where(inArray(actionStockItems.actionId, actionIds)),
      database.select({ actionId: actionSuppliers.actionId, supplierId: suppliers.id, name: suppliers.displayName, photoUrl: suppliers.photoUrl, mainService: suppliers.mainService }).from(actionSuppliers).innerJoin(suppliers, eq(actionSuppliers.supplierId, suppliers.id)).where(inArray(actionSuppliers.actionId, actionIds)),
      database.select({ actionId: actionServices.actionId, serviceTypeId: serviceTypes.id, name: serviceTypes.name, estimatedAmount: actionServices.estimatedAmount, supplierOfferingId: actionServices.supplierOfferingId, offeringName: supplierOfferings.name, unit: supplierOfferings.unit, listedUnitPrice: supplierOfferings.unitPrice, supplierId: suppliers.id, supplierName: suppliers.displayName }).from(actionServices).innerJoin(serviceTypes, eq(actionServices.serviceTypeId, serviceTypes.id)).leftJoin(supplierOfferings, eq(actionServices.supplierOfferingId, supplierOfferings.id)).leftJoin(suppliers, eq(supplierOfferings.supplierId, suppliers.id)).where(inArray(actionServices.actionId, actionIds)),
      database.select({ actionId: auditLogs.entityId, auditAction: auditLogs.action, occurredAt: auditLogs.occurredAt, actorName: users.name, afterData: auditLogs.afterData }).from(auditLogs).leftJoin(users, eq(auditLogs.actorUserId, users.id)).where(and(eq(auditLogs.entityType, "action"), inArray(auditLogs.entityId, actionIds))).orderBy(asc(auditLogs.occurredAt)),
      database.select({ actionId: documents.entityId, url: documents.url, createdAt: documents.createdAt }).from(documents).where(and(eq(documents.entityType, "action"), inArray(documents.entityId, actionIds), inArray(documents.mimeType, ["image/jpeg", "image/png", "image/webp"]))).orderBy(asc(documents.createdAt)),
      database.select({ id: invoices.id, operationId: invoices.operationId, amount: invoices.amount, status: invoices.status }).from(invoices).where(and(eq(invoices.operationType, "action"), inArray(invoices.operationId, actionIds))),
    ]);
    const invoiceIds = invoiceRows.map(invoice => invoice.id);
    const paymentRows = invoiceIds.length ? await database.select({ invoiceId: payments.invoiceId, amount: payments.amount }).from(payments).where(inArray(payments.invoiceId, invoiceIds)) : [];
    return rows.map(row => {
  ...
  create: protectedProcedure.input(z.object({
    ...
    const created = await database.transaction(async transaction => {
      ...
      await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: action.id, action: "create", afterData: { ...action, teamMemberIds: input.teamMemberIds, stockAllocations, serviceAllocations, estimatedCost } }, transaction);
      return action;
    });
    return created;
  }),
  updateDetails: protectedProcedure.input(z.object({
    ...
    const updated = await database.transaction(async transaction => {
      ...
      await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: action.id, action: "update_details", beforeData: before, afterData: { ...action, supplierIds: input.supplierIds, serviceAllocations, teamMemberIds: input.teamMemberIds, stockAllocations } }, transaction);
      return action;
    });
    return updated;
  }),
  uploadCover: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), originalName: z.string().trim().min(1).max(255), mimeType: z.enum(["image/jpeg", "image/png", "image/webp"]), dataBase64: z.string().min(1).max(70_000_000) })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); const database = await requireDatabase(); const bytes = Buffer.from(input.dataBase64, "base64"); if (!bytes.length || bytes.length > 50 * 1024 * 1024) throw new TRPCError({ code: "PAYLOAD_TOO_LARGE", message: "A capa deve ter até 50 MB." }); if (!hasSupportedFileSignature(bytes, input.mimeType)) throw new TRPCError({ code: "BAD_REQUEST", message: "O conteúdo da capa não corresponde ao formato informado." }); const [action] = await database.select({ id: actions.id, cityId: actions.cityId }).from(actions).where(eq(actions.id, input.actionId)); if (!action) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); const safeName = input.originalName.replace(/[^a-zA-Z0-9._-]/g, "_"); const stored = await storagePut(`trade/action/${action.id}/cover-${Date.now()}-${safeName}`, bytes, input.mimeType); const [updated] = await database.update(actions).set({ coverImageUrl: stored.url, updatedAt: new Date() }).where(eq(actions.id, action.id)).returning(); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: action.id, action: "upload_cover", afterData: { url: stored.url } }); return updated; }),
  uploadStatusEvidence: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), originalName: z.string().trim().min(1).max(255), mimeType: z.enum(["image/jpeg", "image/png", "image/webp", "application/pdf", "video/mp4", "video/webm", "audio/mpeg", "audio/wav", "audio/ogg"]), dataBase64: z.string().min(1).max(70_000_000) })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); const database = await requireDatabase(); const bytes = Buffer.from(input.dataBase64, "base64"); if (!bytes.length || bytes.length > 50 * 1024 * 1024) throw new TRPCError({ code: "PAYLOAD_TOO_LARGE", message: "A evidência deve ter até 50 MB." }); if (!hasSupportedFileSignature(bytes, input.mimeType)) throw new TRPCError({ code: "BAD_REQUEST", message: "O conteúdo da evidência não corresponde ao formato informado." }); const [action] = await database.select({ id: actions.id }).from(actions).where(eq(actions.id, input.actionId)); if (!action) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); const safeName = input.originalName.replace(/[^a-zA-Z0-9._-]/g, "_"); const stored = await storagePut(`trade/action-status-evidence/${action.id}/${Date.now()}-${safeName}`, bytes, input.mimeType); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: action.id, action: "upload_status_evidence", afterData: { url: stored.url, mimeType: input.mimeType, sizeBytes: bytes.length } }); return { url: stored.url }; }),
  updateExecutionStatus: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), status: z.enum(["planned", "in_progress", "paused", "completed", "cancelled"]), reason: z.string().trim().max(3000).optional(), evidenceUrls: z.array(evidenceUrlSchema).max(10).default([]) })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); if (["paused", "cancelled"].includes(input.status) && !input.reason) throw new TRPCError({ code: "BAD_REQUEST", message: "Informe o motivo da alteração de status." }); const database = await requireDatabase(); const [before] = await database.select().from(actions).where(eq(actions.id, input.actionId)).limit(1); if (!before) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); if (!canUpdateActionStatus(before.status, input.status)) throw new TRPCError({ code: "BAD_REQUEST", message: "Status de ação inválido." }); return database.transaction(async transaction => { const [updated] = await transaction.update(actions).set({ status: input.status, updatedAt: new Date() }).where(eq(actions.id, input.actionId)).returning(); if (!updated) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: updated.id, action: "update_execution_status", beforeData: before, afterData: { ...updated, reason: input.reason || null, evidenceUrls: input.evidenceUrls } }, transaction); return updated; }); }),
  reschedule: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), scheduledFor: z.coerce.date(), endsAt: z.coerce.date().nullable(), reason: z.string().trim().min(3).max(3000), evidenceUrls: z.array(evidenceUrlSchema).max(10).default([]) })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); if (!validActionRange(input.scheduledFor, input.endsAt)) throw new TRPCError({ code: "BAD_REQUEST", message: "O término da ação deve ser posterior ao início." }); const database = await requireDatabase(); const [before] = await database.select().from(actions).where(eq(actions.id, input.actionId)); if (!before) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); if (["completed", "cancelled"].includes(before.status)) throw new TRPCError({ code: "CONFLICT", message: "Ações concluídas ou canceladas não podem ser reagendadas." }); return database.transaction(async transaction => { const [updated] = await transaction.update(actions).set({ scheduledFor: input.scheduledFor, endsAt: input.endsAt, updatedAt: new Date() }).where(eq(actions.id, input.actionId)).returning(); if (!updated) throw new TRPCError({ code: "NOT_FOUND", message: "Ação não encontrada." }); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action", entityId: updated.id, action: "reschedule", beforeData: before, afterData: { ...updated, reason: input.reason, evidenceUrls: input.evidenceUrls } }, transaction); return updated; }); }),
  saveDebrief: protectedProcedure.input(z.object({ actionId: z.number().int().positive(), rating: z.number().int().min(1).max(5), notes: z.string().trim().max(3000).optional(), positives: z.string().trim().max(2000).optional(), negatives: z.string().trim().max(2000).optional(), resultAchieved: z.boolean(), resultSummary: z.string().trim().max(3000).optional(), leadCount: z.coerce.number().int().min(0).max(9999999).default(0), saleCount: z.coerce.number().int().min(0).max(9999999).default(0), renewalCount: z.coerce.number().int().min(0).max(9999999).default(0), worthRepeating: z.boolean().nullable(), completedAt: z.coerce.date() })).mutation(async ({ ctx, input }) => { await assertPermission(ctx.user, "actions.write"); const database = await requireDatabase(); const [current] = await database.select().from(actionDebriefs).where(eq(actionDebriefs.actionId, input.actionId)); const values = { ...input, notes: input.notes || null, positives: input.positives || null, negatives: input.negatives || null, resultSummary: input.resultSummary || null }; return database.transaction(async transaction => { const [saved] = current ? await transaction.update(actionDebriefs).set(values).where(eq(actionDebriefs.actionId, input.actionId)).returning() : await transaction.insert(actionDebriefs).values(values).returning(); if (!saved) throw new TRPCError({ code: "NOT_FOUND", message: "Não foi possível salvar o debriefing." }); await writeAuditLog({ actorUserId: ctx.user.id, entityType: "action_debrief", entityId: saved.id, action: current ? "update" : "create", afterData: saved }, transaction); return saved; }); }),
});
