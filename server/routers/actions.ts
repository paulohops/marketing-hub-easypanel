...
export const actionsRouter = router({
  referenceData: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [cityRows, typeRows, supplierRows, serviceRows, supervisorRows, teamRows, stockRows, actionPointRows, supplierCityRows, supplierServiceTypeRows, campaignRows, eventRows, offeringRows, templateRows] = await Promise.all([
      ...
      database.select().from(actionTypes).where(eq(actionTypes.active, true)).orderBy(asc(actionTypes.name)).catch(() => []),
      ...
      database.select({ id: users.id, name: users.name, email: users.email, jobTitle: users.jobTitle, avatarUrl: users.avatarUrl }).from(users).where(eq(users.isActive, true)).orderBy(asc(users.name)).catch(() => []),
      ...
      database.select({ supplierId: supplierServiceTypes.supplierId, serviceTypeId: supplierServiceTypes.serviceTypeId }).from(supplierServiceTypes).catch(() => []),
      database.select({ id: tradeCampaigns.id, name: tradeCampaigns.name, regionalId: tradeCampaigns.regionalId, status: tradeCampaigns.status, logoUrl: tradeCampaigns.logoUrl }).from(tradeCampaigns).orderBy(asc(tradeCampaigns.name)).catch(() => []),
      database.select({ id: events.id, name: events.name, cityId: events.cityId, tradeCampaignId: events.tradeCampaignId, status: events.status, startsAt: events.startsAt }).from(events).orderBy(asc(events.startsAt)).catch(() => []),
      database.select({ id: supplierOfferings.id, supplierId: supplierOfferings.supplierId, name: supplierOfferings.name, unit: supplierOfferings.unit, unitPrice: supplierOfferings.unitPrice, notes: supplierOfferings.notes }).from(supplierOfferings).where(and(eq(supplierOfferings.kind, "service"), eq(supplierOfferings.active, true))).orderBy(asc(supplierOfferings.name)).catch(() => []),
      database.select({ template: actionTemplates, actionTypeName: actionTypes.name }).from(actionTemplates).leftJoin(actionTypes, eq(actionTemplates.defaultActionTypeId, actionTypes.id)).where(eq(actionTemplates.active, true)).orderBy(asc(actionTemplates.name)).catch(() => []),
    ]);
