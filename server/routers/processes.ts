...
import { z } from "zod";
import { processes, regionals, users } from "../../drizzle/schema";
...
async function validateReferences(database: Awaited<ReturnType<typeof requireDatabase>>, input: ProcessInput) {
...
export const processesRouter = router({
  ...
  list: protectedProcedure.input(z.object({ search: z.string().trim().max(120).optional(), status: z.enum(processStatuses).optional() }).optional()).query(async ({ ctx, input }) => {
    ...
    const rows = await database.select({ process: processes, ownerName: users.name, regionalName: regionals.name }).from(processes).leftJoin(users, eq(processes.ownerUserId, users.id)).leftJoin(regionals, eq(processes.regionalId, regionals.id)).where(filters.length ? and(...filters) : undefined).orderBy(desc(processes.updatedAt), asc(processes.name));
    return rows.map(row => ({ ...row.process, ownerName: row.ownerName ?? null, regionalName: row.regionalName ?? null }));
  }),
  ...
  get: protectedProcedure.input(z.object({ id: z.number().int().positive() })).query(async ({ ctx, input }) => {
    ...
    return { ...row.process, ownerName: row.ownerName ?? null, regionalName: row.regionalName ?? null };
  }),
  ...
  create: protectedProcedure.input(processFields).mutation(async ({ ctx, input }) => {
    ...
    const [created] = await database.insert(processes).values({ ...data, createdByUserId: ctx.user.id }).returning();
    await writeAuditLog({ actorUserId: ctx.user.id, regionalId: created.regionalId, entityType: "process", entityId: created.id, action: "create", afterData: data });
    return created;
  }),
  ...
  update: protectedProcedure.input(processFields.extend({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    ...
    const [updated] = await database.update(processes).set({ ...data, updatedAt: new Date() }).where(eq(processes.id, id)).returning();
    await writeAuditLog({ actorUserId: ctx.user.id, regionalId: updated.regionalId, entityType: "process", entityId: updated.id, action: "update", beforeData: before, afterData: data });
    return updated;
  }),
});
