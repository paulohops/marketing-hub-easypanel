...
async function ensureEventReferences(database: any, input: { cityId: number; eventTypeId: number; campaignId?: number | null; commercialSupervisorId: number | null; supplierIds: number[]; serviceTypeIds: number[]; teamMemberIds: number[]; stockAllocations: Array<{ stockItemId: number }> }) {
  const checks = [
