...
export function normalizeAppBranding(value: unknown): AppBranding {
  ...
  return {
    ...
    darkBackgroundColor: isHexColor(source.darkBackgroundColor) ? source.darkBackgroundColor.toUpperCase() : DEFAULT_APP_BRANDING.darkBackgroundColor,
...
export const settingsRouter = router({
  ...
  overview: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [providerRows, regionalRows, cityRows, supplierRows, storeRows, partnerRows, serviceRows, mediaTypeRows, actionTypeRows, eventTypeRows, campaignTypeRows, campaignSectorRows, financialCategoryRows, supplierOfferingRows, supervisorRows, actionPointRows, supervisorStoreRows, actionRows, eventRows, mediaPointRows, mediaCampaignRows, actionSupplierRows, eventSupplierRows, providerDocumentRows] = await Promise.all([
      ...
      database.select().from(suppliers).orderBy(asc(suppliers.displayName)).catch(() => []),
      database.select().from(stores).orderBy(asc(stores.name)).catch(() => []),
      ...
      database.select().from(campaignSectors).orderBy(asc(campaignSectors.name)).catch(() => []),
      database.select().from(financialCategories).orderBy(asc(financialCategories.name)).catch(() => []),
      database.select().from(supplierOfferings).orderBy(asc(supplierOfferings.name)).catch(() => []),
      database.select().from(commercialSupervisors).orderBy(asc(commercialSupervisors.name)).catch(() => []),
      database.select().from(actionPoints).orderBy(asc(actionPoints.name)).catch(() => []),
      database.select().from(commercialSupervisorStores).catch(() => []),
      database.select({ id: actions.id, name: actions.name, cityId: actions.cityId }).from(actions).catch(() => []),
      database.select({ id: events.id, name: events.name, cityId: events.cityId }).from(events).catch(() => []),
      database.select({ id: mediaPoints.id, name: mediaPoints.name, cityId: mediaPoints.cityId, supplierId: mediaPoints.supplierId }).from(mediaPoints).catch(() => []),
      database.select({ mediaPointId: mediaCampaigns.mediaPointId }).from(mediaCampaigns).catch(() => []),
      database.select({ actionId: actionSuppliers.actionId, supplierId: actionSuppliers.supplierId }).from(actionSuppliers).catch(() => []),
